...
const PageTemplate = ({ data }) => {
  ...
  const hasChildren = currentPage.wpChildren.nodes.length > 0
  ...
  const parentTitle = hasParent && currentPage.wpParent.node.title
  ...
  return (
    <Layout>
      <Wrapper>
        <ContentWrapper>
          <PageSidebar
            isNested={hasChildren || hasParent}
            parentChildren={parentChildren}
            currentPage={currentPage}
            parentTitle={parentTitle}
          >
            {data.wpPage.wpChildren.nodes}
          </PageSidebar>
          <PageContent>
